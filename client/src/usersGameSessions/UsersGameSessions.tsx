...
function UsersGameSessions({ gameMeets }: { gameMeets: gameMeetsData[] }) {
    ...
    const handleJoin = async (gameMeetingId: number) => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_REACT_APP_API_URL}/meeting/players`, {
                gameMeetingId,
    ...
    return (
        <>
            ...
            <div className="bg-yellow-300 px-6 py-4 rounded-lg flex flex-col items-center justify-center">
              <p className="font-bold text-black mb-4">Контакты организатора:</p>
              <p className="text-black">{organizerContacts}</p>
              <button onClick={closeOrganizerContacts} className="mt-4 px-4 py-2 bg-black text-white rounded-lg">
                Закрыть
              </button>
            </div>
          </div>
