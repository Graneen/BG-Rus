  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <CustomTabPanel value={value} index={1}>
        <div className="recom-head-block mb-10">
          <h2 id="recom-head-block-h2">
            Мы выбираем случайные 10 игр на основании ваших предпочтений
          </h2>
          <NavLink to="/quiz">
            <button className="quiz-button bg-white text-black">
              Изменить выборку игр
            </button>
          </NavLink>
        </div>

        {userRecommendedGames && userRecommendedGames.length
                  ...
                  <div className="profile-image-descr-block">
                    <div className="profile-card-left">
                      <div className="profile-images-section">
                        ...
                        <div className="profile-preview-images">
                          {getImages(userRecommendedGame).map(
                            (photo, index) => (
                              <div
                                key={index}
                                className={
      ...
      <CustomTabPanel value={value} index={2}>
        {userFeedbacks && userFeedbacks.length
          ? userFeedbacks.map((userFeedback) => (
              <div key={userFeedback.game_id} className="mb-8">
                <NavLink to={`/game/${userFeedback.game_id}`}>
                  <div className="mt-8">
                    <h2 className="h2-profile">
                      Игра: {userFeedback.BoardGame.title}
                    </h2>
                  </div>
                </NavLink>
      ...
      <CustomTabPanel value={value} index={3}>
        {userQuestionsAndAnswers && userQuestionsAndAnswers.length
          ? userQuestionsAndAnswers.map((userQuestionAndAnswers) => (
              ...
              >
                <NavLink to={`/game/${userQuestionAndAnswers.game_id}`}>
                  <h2 className="h2-profile">
                    Игра: {userQuestionAndAnswers.game}
                  </h2>
                </NavLink>
                <div className="profile-question">
                  Ваш вопрос: {userQuestionAndAnswers.questions[0].question}
                </div>
                {userQuestionAndAnswers.questions[0].answers.map(
                  (answer, index) => (
                    <div className="profile-answer" key={index}>
                      Ответ: {answer}
                    </div>
                  )
                )}
